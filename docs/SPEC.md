...
Solver結果はcontext一致時のみ採用する。

---

### 9.4 Multiway Postflop

Multiway postflopはeval7 + LLMで判断する。
...
- active_player_count
- num_opponents
- opponent_profiles（total_handsが閾値以上のものだけ）

Actions:
- preflop_action_history
- current_street_action_history

Equity:
- hero_equity
- required_equity
```
`raw_call_amount` と `effective_call_amount` は区別する。

```text
raw_call_amount:
  画面上の相手betとの差分などから見える、額面上のcall額

effective_call_amount:
  Hero stackでcapした実際に支払えるcall額
```
...
`num_opponents` は、stats list長ではなく `active_player_count - 1` を優先する。

禁止:

```text
- Hero stackを超えるcall額でpot oddsを計算すること
- stats list件数をそのままopponent数として使うこと
- preflop action historyとcurrent street action historyを混同すること
```


LLMが `fold` を返しても、hero_equityがrequired_equityを十分に上回る場合は数理ガードでCALLへ補正する。
...
ただし、Solverへ渡す前提情報が不安定な場合はSolverを起動しない。
...
workerは共有result/errorを直接上書きしない。

---

### 10.9 Solver I/O debug保存

Solver input/outputはdebug保存できる。
...
### 10.10 Solver fallback方針

Solverがtimeoutまたはerrorになった場合、fallbackが返ることがある。

ただし、fallbackもcontextが有効な場合のみ採用する。
...
- 古いfallbackを表示
- Hero行動後のfallbackを表示
- NEW_STREET後のfallbackを表示
```


### 10.11 Solver request保存とdeep-SPR比較request

Solverへ送る直前の完全request JSONを `debug/solver_io/YYYYMMDD/` に保存する。
...
deep-SPR flop rootでは、本番requestとは別に比較用requestを保存する。

比較用request:

```text
- flop_bet_sizes_oop/ip から all-in候補 "a" を外す
- turn / river の all-in候補は維持
- Solverへは送らない
- 正式推奨には使わない
```
...
```text
deep-SPR flop timeoutの主因がall-in候補入りtreeにあるかを検証する。
```

本番requestのbet sizeは、比較結果が十分に集まるまで変更しない。
---
