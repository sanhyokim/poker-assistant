...
## §94. サンプリング設計の確定（閾値・canonical圧縮・総量・保存形式）

### 94.0 位置づけ
本節は、P3系列Go（§93）を受けたサンプリング設計フェーズの確定記録である。入力はp3_5、p3_6、sd1成果物である。

P3系列では、multinode CLIによる抽出機構が健全であること、turn/river本番品質は0.5%を維持すること、さらに到達weight下限とノード自己EV矛盾ガードを併用する必要があることが確定した。本節では、その具体的な初期閾値、canonical圧縮方式、目標総量、保存形式を定める。

### 94.1 weight下限とガード閾値の確定
p3_6で、p3_5の80ノードについてweight階級別の自己EV矛盾統計を算出した。normalized weightの安定境目はlog10 >= 2であり、10〜100帯にはP3.4級の変動（56.60→1.98）が残存していた。一方、raw weightはraw >= 1で安定するが、品質分別力がなかった。raw >= 1でも、自己EV矛盾はp90 21.4%、max 39.1% potであり、品質ガードの代替にはならない。

自己EV矛盾ガードを <= 1.0% potとした場合、weight下限位置によらず残存maxは0.984% potとなり、weightカバー率は0.997〜0.999であった。

以上より、初期設定として以下を採用する。

- weight下限: ノードのnormalized weight合計がroot合計の5e-4以上であること。
- 自己EV矛盾ガード: §93.6で定義した自己EV矛盾が1.0% pot以下であること。

weight下限は、4sKcAhの実測ではnormalized weight >= 100相当である。絶対値ではなくroot比で定義することで、ボードごとのレンジ規模差に依存しない。

この設定は保守側の初期値である。失敗時の回復は非対称であり、厳しすぎる場合は追加生成で回収できるが、緩すぎる場合は7日級の生成後に教師汚染が判明し、作り直しになる。下限未満帯の通過率と品質は、パイロットで計測して緩和要否を再判定する。

ガード1.0% potは、flop教師品質（root 0.5%、#24）と桁が整合する唯一の点である。<= 0.5%では80ノード中26件しか残らず過厳であり、<= 2.0%ではflopより明確に緩い教師の混入を許す。なお、この閾値は1ボード実測由来であるため、複数ボード検証はパイロットの必須項目とする（#31）。

### 94.2 SD-1: canonical圧縮の実測基盤
SD-1では、turn/river canonical圧縮の前提を実測した。

まず、pekarstasレンジがスート対称であることを確認した。OOP 534 combo、IP 610 comboについて、全スート置換でmax diffは0.0であった。

次に、1,755 canonical flopそれぞれについて残存スート対称群を数え上げた。分布は以下である。

- size 1: 286 flop
- size 2: 1,170 flop
- size 6: 299 flop

この残存対称性に基づくcanonical種数は、turnでは85,995から63,193へ減少し、26.5%削減であった。turn+riverでは4,127,760から2,526,576へ減少し、38.8%削減であった。

さらに、solver出力上の同型性も実測した。AcKcQc（残存対称size 6）を0.5%でsolveし、turn 2dとturn 2hの同型ノードを比較した。solve実測はexploitability 0.4822%、210 iter、122.9秒、4.18GBであった。
...
ただし、実証は1 flop、パッシブライン1組である。したがって、パイロットに「別flop・bet入りラインの同型スポット確認」を1件追加する。そこで差分0が再確認できれば、本番では代表1枚のみ抽出する。

### 94.4 総量と配分
turn/river教師では、1ノードから手番プレイヤーの全handを展開する。1ノードあたりの教師例数は概ね450〜510例である。

初期目標総量は、turn 600万例、river 600万例、合計1,200万例とする。これは約2.2万〜3.3万ノードに相当し、1,755 board平均では13〜19ノード/boardである。

street単純比例配分は採らない。riverはノード種類が多いが、1ノードあたりの情報が浅い。したがって、初期値はturn/riverを同量とし、配分係数の最終化はパイロット実測（通過率・生成速度）で行う。

派生課題として、データローダー高速化の優先度が上がる。現行のデータロードは180万例で実測47分であり、単純比例では1,200万例で約5時間/ロードとなる。したがって、高速化は「可能性」から「ほぼ必須」に格上げする。

### 94.5 保存形式
本番生成では、ガードとweight下限をその場で判定し、不合格ノードは保存しない。161KB/ノード級の生responseは蓄積しない。

合格ノードについても、保存するのは教師に必要なフィールドに限定する。
...
- pot
- effective_stack
- available_actions（新スキーマ語彙、絶対額、pot比）
- 手番hands
- label確率
- 監査メタ（ノード到達weight、自己EV矛盾値、solve exploitability）

action_ev_matrixと相手側weightsは、ガード計算後に破棄する。監査用にはスカラー値のみを残す。

保存サイズの目標は数KB/ノード台である。正確なサイズはconverter設計時に実測する。
